...
/// <summary>
/// Repository for retrieving weather data from the OpenMeteo API.
/// </summary>
public class OpenMeteoRepository : IWeatherDataRepository
{
    ...
    private async Task<Coordinates> GetCityCoordinates(string city, CancellationToken ct)
    {
        ...
        {
            throw new InvalidCoordinatesForCityException(city, result.Latitude, result.Longitude);
        }
        ...
        CancellationToken ct)
    {
        ...
        {
            throw new InconsistentWeatherDataException(response.Daily.Time.Count, response.Daily.MaxTemp.Count);
        }
